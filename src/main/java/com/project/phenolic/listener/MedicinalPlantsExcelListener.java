 ...
 */
@Slf4j
public class MedicinalPlantsExcelListener extends AnalysisEventListener<Map<Integer, String>> {
     ...
     * 缓存的数据
     */
    private List<MedicinalPlantsEnhancedImportDTO> cachedDataList = new ArrayList<>();
    ...
    public MedicinalPlantsExcelListener(ExcelFieldMapping fieldMapping, DataProcessor dataProcessor) {
    ...
    public void invoke(Map<Integer, String> data, AnalysisContext context) {
        ...
        try {
            MedicinalPlantsEnhancedImportDTO dto = convertToDTO(data, context.readRowHolder().getRowIndex());
            if (dto != null && dto.hasValidData()) {
     ...
     * 将Map数据转换为DTO对象
     */
    private MedicinalPlantsEnhancedImportDTO convertToDTO(Map<Integer, String> data, int rowIndex) {
        MedicinalPlantsEnhancedImportDTO dto = new MedicinalPlantsEnhancedImportDTO();
        ...
        try {
            ...
            if (validationError != null) {
                String errorMsg = String.format("第%d行数据验证失败：%s", rowIndex + 1, validationError);
                errorMessages.add(errorMsg);
                return null;
            }
     ...
     * 设置字段值
     */
    private void setFieldValue(MedicinalPlantsEnhancedImportDTO dto, String fieldName, String value, int rowIndex) {
        try {
            Field field = MedicinalPlantsEnhancedImportDTO.class.getDeclaredField(fieldName);
    ...
    public interface DataProcessor {
        void process(List<MedicinalPlantsEnhancedImportDTO> dataList);
    }
}
